...
y_1, y_2, y_3, y_4 = tf.placeholder("float", [None, 10]), tf.placeholder("float", [None, 10]), tf.placeholder("float", [None, 10]), tf.placeholder("float", [None, 10])
...
_1, _2, _3, _4 = tf.argmax(h_fc21, 1), tf.argmax(h_fc22, 1), tf.argmax(h_fc23, 1), tf.argmax(h_fc24, 1)
...
print '生成数据'
i = 0
for (batch_data, batch_label) in OCRIter(100000, batch_size, 4, 40, 80):
    print '训练模型'
    if i%100 == 0:
